//MAKE LINE 245 in.close() as comment!!!!!!
package lab8;
...
public class GameManagerNew 
{
    ...
    { 
        return GameManagerNew.NO_OF_COLUMNS;
    }
    ...
    { 
        return GameManagerNew.NO_OF_ROWS;
    }
        ...
        for(int y=0; y <= TARGET_Y; y++){
            ...
            for(int x=0; x <= TARGET_X; x++){
                ...
                if(monster.getX()== x && monster.getY() == y){
                    ...
                    toPrint += Character.toString(monster.getIcon());
                    checkMonster = true;
                }
                if(player.getX()== x && player.getY() == y){
                    toPrint = toPrint.trim();
                    toPrint += Character.toString(player.getIcon());
                    checkPlayer = true;
                }
                if(bug.getX()== x && bug.getY() == y){
                    toPrint = toPrint.trim();
                    toPrint += Character.toString(bug.getIcon());
                    checkBug = true;
                }

                if(checkBug == true && checkMonster == true){
                    toPrint = toPrint.charAt(0) + " " + toPrint.charAt(1);
                }
                else if(checkBug == true && checkPlayer == true){
                    toPrint = " " + toPrint;
                }
                else if(checkMonster == true && checkPlayer == true){
                    toPrint =  toPrint + " " ;
                }
                else{
                    toPrint = " " + toPrint + " "; 
    ...
    public void moveObjects()
    {
        ...
        int dieWith3Sides = GameManagerNew.random.nextInt(0,3);
    ...
    {
        GameManagerNew gm = new GameManagerNew();
        ...
        while (!gm.isGameOver() && dirChar != 'q') {            
            ...
            {
                player.changeDirection(GameManagerNew.charToDirection(dirChar));
            }
